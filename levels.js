...
const buildBackground = (sxCol, syRow) => {
    ...
    for (const row of Array(12).keys()) {
        runOfBlocks(sxCol, syRow, row, 0, 16, true, false, false);
    }
    ctx.restore();
}
let courseMap;
...
function loadLevelFromMap(levelMap) {
    ...
    courseMap = new class {
        constructor() {
            ...
            console.log("loading image data ", this.img.width, this.img.height);
    ...
    // check for pixel color equality. We don't care about alpha. (atleast not right now.)
